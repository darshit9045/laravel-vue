...
@section('content_header')
    <h1 class="font-weight-bold border-bottom pb-3 d-flex align-items-center justify-content-between">Product <a class="btn btn-info btn-sm" href="{{ route('products.create') }}"><i class="fa fa-plus pr-1" aria-hidden="true"></i>Add product</a></h1>
@stop
        ...
        <div class="row justify-content-between">
            <table class="table table-border table-hover">
                ...
                @forelse ($product as $prd)
                    <tr>
                        <td><img src="{{ asset('images/products/'.$prd->product_image) }}" width="150"></td>
                        <td>{{ $prd->title }}</td>
                        <td>{{ $prd->short_title }}</td>
                        <td class="d-flex">
                            <a class="btn btn-primary mr-2" href="{{ route('products.edit',$prd->id) }}">
